...
void ATPSCharacter::EquipWeaponAtSlot(int slot)
{
	...
	UTexture2D* icon = CurrentWeapon->weaponIcon;
	float ammo = CurrentWeapon->GetAmmoCount();
	float mag = CurrentWeapon->GetMagazineSize();
	TriggerOnWeaponAmmoChange(icon, ammo, mag);
}
